# AtlasHarness Python 版：可恢复、可治理、可进化的 Agent Harness
...
Schema：Pydantic v2（JSON Schema 和运行时校验）
持久化：sqlite3/aiosqlite + SQLite + append-only JSONL
向量检索：先 BM25，后接 SQLite 向量扩展或独立向量库
MCP：标准 MCP Client/Server（Python MCP SDK）
CLI：Typer，后续接 Textual TUI
HTTP：FastAPI + Uvicorn 或等价轻量 HTTP 层
测试：pytest + pytest-asyncio + 集成测试 + 回放测试
```

选择 Python 是因为 asyncio、Pydantic 和 FastAPI 能直接覆盖异步运行时、边界校验和服务化需求，同时便于复用评测与数据分析生态。核心状态机使用类型标注和显式协议保持边界清晰。
