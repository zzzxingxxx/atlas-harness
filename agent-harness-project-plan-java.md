# AtlasHarness Java 版：可恢复、可治理、可进化的 Agent Harness
...
```text
Runtime：Java 21 LTS（虚拟线程、records、sealed interfaces）
构建：Gradle 8.x
Schema：Jackson + NetworkNT JSON Schema（运行时校验）
持久化：SQLite JDBC + append-only JSONL
向量检索：Apache Lucene BM25，后接 SQLite 向量扩展或独立向量库
MCP：标准 MCP Client/Server（Java MCP SDK）
CLI：picocli，后续接 Lanterna TUI
HTTP：Javalin 或等价轻量 HTTP 层
测试：JUnit 5 + AssertJ + 集成测试 + 回放测试
```

选择 Java 21 是因为虚拟线程、records 和 sealed interfaces 能支撑高并发异步操作、不可变事件模型和清晰的类型边界。Jackson、SQLite JDBC、Lucene 与 picocli 覆盖序列化、持久化、检索和 CLI 需求，评测 Worker 可复用同一套 Java 模块。
...
public interface ModelAdapter {
    java.util.concurrent.Flow.Publisher<ModelEvent> stream(ModelRequest request);
    java.util.concurrent.CompletionStage<Integer> countTokens(TokenInput input);
...
public record ToolManifest(
    ...
    public enum Risk { READ, WRITE, NETWORK, DESTRUCTIVE }
}
```
